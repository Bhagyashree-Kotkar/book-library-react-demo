...
import { removeFromcart } from '../../services/user.services';
...
export default function Carts() {
  ...
  // first data grab
    useEffect(() => {
      fetch('http://localhost:3001/Cart') // your url may look different
        .then(resp => resp.json())
    ...
    <div>
      ...
      <CardActions sx={{ alignItems: 'center' }}>
        <Box textAlign='center'>
            <Button onClick={(e) => {
              removeFromcart(book.id,e).then(res =>  {
                  if(res.status === 200) {
                  swal('Book Removed From Cart');
                  navigate(0); 
                } else {
                  swal(res.json());
                  navigate(0); 
              ...
              }} size="small">Remove From Cart</Button>
        </Box>
      </CardActions>
    </Card>
    </Grid>
